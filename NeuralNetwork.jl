...
#     reset_net!(model)
#     ŷ_order = model(x[  1:196,:])
#     ŷ_order = model(x[197:392,:])
#     ŷ_order = model(x[393:588,:])
#     ŷ_order = model(x[589:end,:])
#     y_var = Variable(y, name="y")
#     loss_order = AutoDiff.crossentropy(AutoDiff.softmax(ŷ_order), y_var)
#     loss_order = topological_sort(loss_order)
